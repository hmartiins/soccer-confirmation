...
final class NextEventRxPresenter implements NextEventPresenter {
  ...
  NextEventViewModel _mapEvent(NextEvent event) => NextEventViewModel(
        ...
        goalkeepers:
            _mapPlayers(_goalkeepers(event.players).sortedBy(_sortByDate)),
  ...
  List<NextEventPlayerViewModel> _mapPlayers(List<NextEventPlayer> player) =>
      player.map(_mapPlayer).toList();
}
